...
2. Ganti isinya dengan:
   ...
   service cloud.firestore {
     match /databases/{database}/documents {
       ...
       match /tamanSettings/{docId} {
         allow read: if true;
         allow write: if request.auth != null;
       }
       match /tamanPlants/{plantId} {
         allow read: if true;
         allow write: if request.auth != null;
       }
       match /tamanKegiatan/{kegiatanId} {
         allow read: if true;
         allow create: if true;
         allow update, delete: if request.auth != null;
       }
       match /tamanPiket/{piketId} {
         allow read: if true;
         allow create: if true;
         allow update, delete: if request.auth != null;
       }
...
>
> Untuk `tamanKegiatan` (Selasa Asri) dan `tamanPiket` (Piket Harian),
> aturannya sengaja dibuat sedikit berbeda: **`allow create: if true`**
> artinya siapa saja boleh **menambah** dokumentasi kegiatan baru tanpa
> perlu login (karena petugas piket & Selasa Asri berganti-ganti setiap
> hari/minggu), tapi **mengubah atau menghapus** entri yang sudah ada tetap
> butuh login admin. Katalog `tamanPlants` (daftar tanaman) dan
> `tamanSettings` (sambutan taman) tetap sepenuhnya dikelola admin, sama
> seperti `tasks`.
...
| `tamanSettings`     | 1 dokumen tetap dengan id `welcome`: sambutan menu Taman, field `title, caption, photo` (foto sampul, base64) |
| `tamanPlants`       | Katalog tanaman, field `name, latin, category, location, desc, photos` (array foto, base64) — dikelola admin, dipakai juga untuk generate QR Code label tanaman |
| `tamanKegiatan`     | Dokumentasi kegiatan "Selasa Asri", field `date, petugas, title, desc, photos, createdAt` — siapa saja boleh menambah tanpa login |
| `tamanPiket`        | Catatan piket harian taman, field `date, petugas, desc, photos, createdAt` — siapa saja boleh menambah tanpa login |
...
Foto pada menu Taman (sambutan, tanaman, kegiatan Selasa Asri, piket harian)
memakai mekanisme kompresi & penyimpanan yang persis sama.

## Catatan: Menu Taman — QR Code Tanaman

Setiap tanaman di **Daftar Tanaman** otomatis mendapat QR Code (dibuat di
browser memakai library `qrcodejs`, dimuat lewat CDN di `index.html`). Isi
QR Code adalah link permanen ke situs ini dengan parameter `?tanaman=<id>`.
Saat QR itu dipindai (mis. pakai kamera HP), situs akan otomatis terbuka
langsung ke menu Taman dan menampilkan detail tanaman terkait — jadi
pastikan situs sudah di-hosting (lihat bagian Hosting) sebelum mencetak &
menempel QR di taman, supaya link di dalam QR benar-benar bisa dibuka orang
lain. Tombol **"Cetak Label"** pada detail tanaman membuka jendela cetak
berisi QR + nama tanaman, siap dicetak dan ditempel di dekat tanaman
tersebut. Tombol **"Unduh QR"** menyimpan QR sebagai file gambar PNG.
